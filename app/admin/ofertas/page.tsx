...
export default function OfertasPage() {
  ...
  const handleToggleDeal = async (product: Product) => {
    ...
    const newVal = !product.is_deal
    await supabase.from('products').update({ is_deal: newVal, updated_at: new Date().toISOString() }).eq('id', product.id)
    setProducts((prev) => prev.map((p) => (p.id === product.id ? { ...p, is_deal: newVal } : p)))
    setToggling(null)
  }
  ...
  const filtered = products.filter((p) => {
    ...
    const matchTab =
      tab === 'todos' || (tab === 'habilitados' && !!p.is_deal) || (tab === 'nao-habilitados' && !p.is_deal)
    return matchSearch && matchTab
  })

  const habilitados = products.filter((p) => p.is_deal).length

  return (
    <AdminShell title="Ofertas do Dia">
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">Ofertas do Dia</h2>
            <p className="text-gray-500 text-sm mt-0.5">{habilitados} produtos ativos nas Ofertas do Dia</p>
          </div>
          <Link
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex gap-3">
          ...
          <p className="text-amber-800 text-sm">
            Ative ou desative a opção <strong>Oferta do Dia</strong> em cada produto. Os produtos habilitados aparecem automaticamente na seção &quot;Ofertas do Dia&quot; da Home, sem necessidade de cadastro separado.
          </p>
        </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    ...
                    <th className="text-left px-4 py-3 font-medium text-gray-600">Oferta do Dia</th>
                  ...
                  {filtered.map((p) => (
                    <tr key={p.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3">
                        ...
                        >
                          ⚡ {p.is_deal ? 'Habilitado' : 'Desabilitado'}
                        </button>
                      </td>
                      <td className="px-4 py-3">
                        <button
